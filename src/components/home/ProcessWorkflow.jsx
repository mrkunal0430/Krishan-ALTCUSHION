...
const steps = [
  {
    id: 1,
    title: 'Strategy',
    description: 'We begin by deconstructing your vision. Through deep market research and competitor analysis, we architect a roadmap that aligns your business goals with technological feasibility.',
    icon: Lightbulb,
  },
  {
    id: 2,
    title: 'Design',
    description: 'Our design philosophy blends aesthetics with function. We create intuitive, high-fidelity prototypes and UI systems that ensure seamless user experiences before a single line of code is written.',
    icon: PenTool,
  },
  {
    id: 3,
    title: 'Develop',
    description: 'We engineer robust, scalable solutions using cutting-edge stacks. Our agile development process ensures iterative delivery, clean code, and security-first architecture.',
    icon: Code2,
  },
  {
    id: 4,
    title: 'Support',
    description: 'Launch is just the beginning. We provide 24/7 monitoring, performance optimization, and continuous updates to ensure your digital infrastructure grows with your business.',
    icon: Headset,
  }
];
       ...
       <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">
            Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">Workflow</span>
          </h2>
          <p className="text-slate-400 max-w-2xl mx-auto">
            A precision-engineered process designed to transform abstract ideas into market-dominating digital products.
          </p>
        </div>
    ...
    >
      ...
      {isActive && (
        <motion.div 
          ...
          className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-orange-500 to-red-600 shadow-[0_0_10px_2px_rgba(249,115,22,0.5)]"
        />
      )}

      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center space-x-4">
           <span className={`text-sm font-mono font-bold ${isActive ? 'text-orange-400' : 'text-slate-500'}`}>
             0{step.id}
           </span>
        ...
        <span className={`text-xs uppercase tracking-wider ${isActive ? 'text-red-400' : 'text-slate-600'}`}>
          Step {step.id} of 4
        </span>
      </div>
    </motion.div>
          ...
          {[1, 2, 3, 4].map((i) => (
             <div 
               ...
               className={`h-full flex-1 rounded-full transition-all duration-500 ${
                 i <= step.id ? 'bg-gradient-to-r from-orange-500 to-red-500' : 'bg-transparent'
               }`} 
             />
          ))}
        </div>
